...
if [ -n "$TG_ARN" ]; then
    ...
    if [ "$HEALTHY_COUNT" -lt 2 ]; then
        ...
        # Show detailed target health
        echo "   Target details:"
        aws elbv2 describe-target-health --target-group-arn "$TG_ARN" --query "TargetHealthDescriptions[*].[Target.Id,TargetHealth.State,TargetHealth.Reason]" --output table 2>/dev/null || echo "   Could not retrieve target details"
    fi
else
...
if [ -n "$ALB_DNS" ]; then
    ...
    if [ "$HTTP_CODE" = "200" ]; then
        ...
        # Try to identify which server responded
...
if [ -n "$VPC_ID" ]; then
    ...
    if [ "$SUBNET_COUNT" -ge 4 ]; then
        ...
        aws ec2 describe-subnets --filters "Name=vpc-id,Values=$VPC_ID" --query "Subnets[*].[Tags[?Key=='Name'].Value | [0], CidrBlock, AvailabilityZone]" --output table 2>/dev/null || true
    else
